...
import { ADMIN_HASHES, DEFAULT_MEMBERS, DEFAULT_NEWS, DEFAULT_ARTICLES, DEFAULT_REPORTS } from "./content.js";
...
function applyChrome() {
  ...
  document.documentElement.classList.toggle("is-staff", Boolean(deskRole()));
...
function loadContent() {
  try {
    ...
    if (raw?.members && raw?.news) {
      return {
        members: raw.members,
        news: raw.news,
        articles: raw.articles || DEFAULT_ARTICLES.map((x) => ({ ...x })),
        reports: raw.reports || DEFAULT_REPORTS.map((x) => ({ ...x })),
      };
    }
  } catch { /* ignore */ }
  return {
    ...
    articles: DEFAULT_ARTICLES.map((x) => ({ ...x })),
    reports: DEFAULT_REPORTS.map((x) => ({ ...x })),
  };
}
...
function hashRoute() {
  ...
  return ["home", "learn", "cycle", "save", "life", "map", "sdg6", "quiz", "pledge", "pledges", "about", "sources", "team", "news", "articles", "reports", "signin", "edit"].includes(h) ? h : "home";
}
...
function googleCampus(mode) {
  const hl = state.lang === "hi" ? "hi" : "en";
  const layer = mode === "3d" ? "k" : "m";
  const z = mode === "3d" ? 17 : 16;
  return `https://maps.google.com/maps?q=National%20Law%20University%20Odisha%20Naraj%20Cuttack&t=${layer}&z=${z}&hl=${hl}&output=embed`;
}

function pageMap() {
  const mode = state.view === "3d" ? "3d" : "2d";
  return hero(TX.mapK, t(TX.mapT), { en: "Live Google Map of NLU Odisha, Naraj. 2D is the street plan; 3D is satellite with the campus area marked.", hi: "एनएलयूओ नराज का लाइव गूगल मानचित्र। 2D सड़क योजना; 3D उपग्रह और चिह्नित परिसर।" }) + `<div class="wrap">
    <div class="glass glass-card map-frame">
      <span class="glass glass-pill glass-hud">${mode === "3d" ? "3D · satellite" : "2D · Google"}</span>
      <iframe title="NLU Odisha on Google Maps" loading="lazy" referrerpolicy="no-referrer-when-downgrade" src="${googleCampus(mode)}"></iframe>
      ${mode === "3d" ? `<div class="map-highlight"><span>NLU Odisha</span></div>` : ""}
    </div>
    <p style="margin-top:.8rem"><a class="btn" href="https://www.google.com/maps/search/?api=1&query=National+Law+University+Odisha+Naraj+Cuttack" target="_blank" rel="noreferrer">${t(TX.openGmaps)}</a></p>
  </div>`;
}
...
function listCards(items, empty) {
  if (!items.length) return `<p class="muted">${empty}</p>`;
  return items.map((n) => `<article class="glass glass-card pad" style="margin-bottom:.75rem">
    <p class="kicker">${n.month || n.by || ""}</p>
    <h2>${n.title}</h2>
    <p class="muted">${n.body || ""}</p>
    ${n.url ? `<p><a class="btn ghost glass glass-pill" href="${n.url}" target="_blank" rel="noreferrer">${t(TX.openSource)}</a></p>` : ""}
  </article>`).join("");
}

function pageArticles() {
  const items = (loadContent().articles || []).filter((n) => n.published !== false);
  return hero(TX.articlesK, t(TX.articlesT), TX.articlesL) + `<div class="wrap">${listCards(items, t(TX.noItems))}</div>`;
}

function pageReports() {
  const items = (loadContent().reports || []).filter((n) => n.published !== false);
  return hero(TX.reportsK, t(TX.reportsT), TX.reportsL) + `<div class="wrap">${listCards(items, t(TX.noItems))}</div>`;
}

function pagePledges() {
  const wall = loadWall();
  return hero(TX.pledgeWallK, t(TX.pledgesT), TX.pledgeWallL) + `<div class="wrap">
    <article class="glass glass-card pad">
      <p class="stat">${wall.length.toLocaleString(state.lang === "hi" ? "hi-IN" : "en-IN")}</p>
      <div class="pledge-wall">${wall.length ? wall.map((p) => `<span class="chip">${displayName(p.name)}</span>`).join("") : `<span class="muted">${t(TX.noPledges)}</span>`}</div>
      <p style="margin-top:1rem"><a class="btn" href="#/pledge">${t(TX.iWill)}</a></p>
    </article>
  </div>`;
}
...
function pageSignin() {
  if (deskRole()) {
    return hero(TX.staff, t(TX.edit), TX.staffL) + `<div class="wrap" style="max-width:28rem">
      <a class="btn" href="#/edit">${t(TX.edit)}</a>
      <button class="btn ghost" id="desk-out" style="margin-left:.5rem">${t(TX.signOutDesk)}</button>
    </div>`;
  }
  return hero(TX.staff, t(TX.staff), TX.staffL) + `<div class="wrap" style="max-width:28rem">
    <p class="muted">${t(TX.authNeed)}</p>
    <p class="row">
      <button class="btn ghost" id="google-in" type="button">${t(TX.googleIn)}</button>
      <button class="btn ghost" id="x-in" type="button">${t(TX.xIn)}</button>
    </p>
    <form class="glass glass-card pad" id="desk-form" style="margin-top:1rem">
...
function pageEdit() {
  ...
  return hero(TX.desk, t(TX.edit), TX.deskL) + `<div class="wrap">
    ...
    <div class="grid grid-2">
      ...
      <section class="glass glass-card pad">
        <h2>${t(TX.articlesT)}</h2>
        <div id="article-list">${(data.articles || []).map((n, i) => extraForm(n, i, "article")).join("")}</div>
        <button class="btn ghost" id="article-add" type="button">${t(TX.addNews)}</button>
      </section>
      <section class="glass glass-card pad">
        <h2>${t(TX.reportsT)}</h2>
        <div id="report-list">${(data.reports || []).map((n, i) => extraForm(n, i, "report")).join("")}</div>
        <button class="btn ghost" id="report-add" type="button">${t(TX.addNews)}</button>
      </section>
    </div>
  </div>`;
}
...
function newsForm(n, i) {
  return extraForm(n, i, "news");
}

function extraForm(n, i, kind) {
  return `<div class="member-edit glass glass-card pad" data-${kind}="${i}" data-id="${esc(n.id || "")}">
    ...
    <button type="button" class="btn ghost" data-remove-${kind}>${t(TX.remove)}</button>
  </div>`;
}
...
function readDeskForms() {
  ...
  const collect = (sel) => [...document.querySelectorAll(sel)].map((el) => ({
    id: el.dataset.id || "x-" + Math.random().toString(36).slice(2, 8),
    month: el.querySelector("[name=month]")?.value || "",
    title: el.querySelector("[name=title]")?.value.trim() || "",
    body: el.querySelector("[name=body]")?.value.trim() || "",
    url: el.querySelector("[name=url]")?.value.trim() || "",
  ...
  return { members, news, articles: collect("[data-article]"), reports: collect("[data-report]") };
}
...
const pages = {
  ...
  team: pageTeam, news: pageNews, articles: pageArticles, reports: pageReports,
  pledges: pagePledges, signin: pageSignin, edit: pageEdit,
};
...
export async function paint() {
  ...
  try {
    ...
    if (editLink) {
      editLink.textContent = deskRole() ? t(TX.edit) : t(TX.staff);
      editLink.href = deskRole() ? "#/edit" : "#/signin";
      editLink.classList.toggle("staff-only", !deskRole());
      if (deskRole()) editLink.classList.remove("staff-only");
    }
    try {
...
function onClick(e) {
  ...
  if (e.target.id === "article-add" || e.target.id === "report-add") {
    const data = readDeskForms();
    const now = new Date();
    const item = {
      id: "x-" + Date.now(),
    ...
    if (e.target.id === "article-add") data.articles.unshift(item);
    else data.reports.unshift(item);
    saveContent(data);
    paint();
    return;
  }
  if (e.target.closest("[data-remove-article]")) {
    e.target.closest("[data-article]")?.remove();
    saveContent(readDeskForms());
    paint();
    return;
  }
  if (e.target.closest("[data-remove-report]")) {
    e.target.closest("[data-report]")?.remove();
    saveContent(readDeskForms());
    paint();
    return;
  }
  if (e.target.id === "google-in" || e.target.id === "x-in") {
