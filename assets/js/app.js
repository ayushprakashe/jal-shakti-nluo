import {
  ...
  AZ, CAMPUS, RESOURCES, QUIZ_BANK, TAGLINE,
} from "./data.js";
...
function pageHome() {
  ...
  ].map(([id, lab]) => `<a class="glass glass-card pad" href="#/${id}" style="text-decoration:none">${lab}</a>`).join("");
  return `
    <section class="hero">
    ...
    <div class="wrap">
      ${sceneOr2d("home", `<div class="blob"></div><p class="note">${t(TX.viewHint2d)}</p>`)}
...
function mapSvg() {
  const pins = CAMPUS.map((b) => {
    ...
    return `<g class="hot ${on ? "active" : ""}" data-pin="${b.id}">
      <rect x="${b.x}" y="${b.y}" width="${b.w}" height="${b.h}" rx="8" fill="${b.color}" opacity="${on ? 1 : 0.88}" stroke="${on ? "#eef7f3" : "transparent"}" stroke-width="3"/>
      <text x="${b.x + b.w / 2}" y="${b.y + b.h / 2 + 4}" text-anchor="middle" fill="#04182a" font-size="11" font-weight="600">${t(b)}</text>
    </g>`;
  }).join("");
  return `<svg class="map-svg" viewBox="0 0 1000 720">
    <rect x="0" y="0" width="1000" height="720" rx="22" fill="#0a2a28"/>
    <path d="M0 40 C200 10,400 80,620 30 S900 20,1000 70 L1000 150 C700 110,400 180,0 130 Z" fill="#1a6a88" opacity=".85"/>
    <text x="40" y="88" fill="#c8f0ff" font-size="13">${state.lang === "hi" ? "काठजोड़ी / महानदी पट्टी" : "Kathajodi / Mahanadi belt"}</text>
    <rect x="470" y="200" width="18" height="430" rx="6" fill="#2a3338"/>
    <text x="20" y="700" fill="#b4cfc6" font-size="11">Naraj Road</text>
    ${pins}
  </svg>`;
}
...
function pagePledge() {
  ...
  if (saved?.name) {
    return hero(TX.pledgeK, TAGLINE, TX.pledgeL) + `<div class="wrap" style="max-width:36rem">
      <article class="glass glass-card pad">
...
function pageSources() {
  return hero({ en: "Sources", hi: "स्रोत" }, t(TX.sourcesT), { en: "Official and established references.", hi: "आधिकारिक संदर्भ।" }) +
    `<div class="wrap grid grid-2">${RESOURCES.map(([n, href]) => `<a class="glass glass-card pad" href="${href}" target="_blank" rel="noreferrer" style="text-decoration:none"><strong>${n}</strong></a>`).join("")}</div>`;
}
...
async function mountScenes() {
  ...
  const three = await import("./three-app.js");
  boxes.forEach((el) => {
    const id = el.dataset.scene;
    if (id === "home") three.homeScene(el);
    if (id === "cycle") three.cycleScene(el, (sid) => { state.cycle = sid; paint(); });
    if (id === "life") three.lifeScene(el);
    if (id === "save") three.saveScene(el, (rid) => { state.room = rid; paint(); });
    if (id === "campus") three.campusScene(el, (pid) => { state.pin = pid; paint(); });
  });
}
...
export async function paint() {
  ...
  try {
    const three = await import("./three-app.js");
    if (state.view !== "3d") three.killAll();
